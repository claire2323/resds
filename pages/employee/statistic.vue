	...
	<view class="container">
		...
		<view class="filter-section">
			<view class="select-row">
				<uni-data-select class="select-item" v-model="formData.serviceTypeId" :localdata="serve_state"
					@change="changeService" placeholder="服务类型">
				</uni-data-select>
				<uni-data-select class="select-item" v-model="formData.orderState" :localdata="order_state"
					@change="changeOrder" placeholder="订单状态">
				</uni-data-select>
				<uni-data-select class="select-item" v-model="formData.timeFlag" :localdata="time_state"
					@change="changeTime" placeholder="接单时间">
				</uni-data-select>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				...
				try {
					const {
						code,
						data
					} = await getOrderList('SERVICE', this.formData)
					if (code == 200) {
			...
			toDetail(id) {
				uni.navigateTo({
					url: `/pages/employee/overDetail?id=${id}`,
				});
			},
